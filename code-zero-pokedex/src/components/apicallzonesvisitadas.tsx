...
const ApiCycleComponentZonesVisitadas: React.FC = () => {
  ...
  const organizePokemonsByLocation = (pokemons: any[]) => {
    ...
    // Obtener las zonas guardadas en localStorage
    const savedZones = JSON.parse(localStorage.getItem("savedZones") || "[]");

    // Filtrar las zonas que están guardadas en el localStorage
    const filteredZones = pokemons.flatMap((pokemon) =>
      pokemon.location_area_encounters.filter((zoneId: string) =>
        savedZones.includes(zoneId)
      )
    );

    // Organizar Pokémon por sus zonas de encuentro filtradas
    filteredZones.forEach((zoneId) => {
      ...
      pokemons.forEach((pokemon) => {
        if (pokemon.location_area_encounters.includes(zoneId)) {
...
export default ApiCycleComponentZonesVisitadas;
